...
{
    return (mTable && mTable->base) ? mTable->base->Index() : 0;
}
...
void rvEnvParms1::Init(const rvEnvParms& copy, float duration)
{
    ...
    if (copy.mIsCount)
        this->mRate = copy.mRate / duration;
    else
...
{
    if (mTable) {
        const float t = time * mRate + mEnvOffset;
        *dest = (mEnd - mStart) * TableSample(mTable, t) + mStart;
    }
    else {
        *dest = mStart;
    }
